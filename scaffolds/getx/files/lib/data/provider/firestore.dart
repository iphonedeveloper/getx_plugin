...
//https://dlohani.com.np/blog/flutter-recipes-2-firebase-firestore-helpers
...
class FirestoreService<T> {
  ...
  FirestoreService(this.basePath, {required this.fromDS, required this.toMap});

  Future<T?> getSingle(String id) async {
    var snap = await _db.collection(basePath).doc(id).get();
  ...
  Stream<List<T>> streamList() {
    var ref = _db.collection(basePath);
  ...
  Future<List<T>> getQueryList({
    ...
    CollectionReference collref = _db.collection(basePath);
    Query? ref;
    if (args != null) {
    ...
    return query.docs.map((doc) => fromDS(doc.id, (doc.data() as Map<String, dynamic>))).toList();
  }
  ...
  Stream<List<T>> streamQueryList({
    ...
    if (ref != null) {
      return ref.snapshots().map((snap) =>
          snap.docs.map((doc) => fromDS(doc.id, (doc.data() as Map<String, dynamic>))).toList());
    } else {
  ...
  Future<List<T>> getListFromTo(String field, DateTime from, DateTime to,
    ...
    var ref = _db.collection(basePath).orderBy(field);
    for (final arg in args) {
      if (arg is QueryArgsV2) {
        ref = ref.where(
          arg.key,
  ...
  Stream<List<T>> streamListFromTo(String field, DateTime from, DateTime to,
    ...
    var ref = _db.collection(basePath).orderBy(field, descending: true);
    for (final arg in args) {
      if (arg is QueryArgsV2) {
        ref = ref.where(
          arg.key,
  ...
  Future<dynamic> createItem(T item, {String? id}) {
    if (id != null) {
      return _db.collection(basePath).doc(id).set(toMap(item));
    } else {
      return _db.collection(basePath).add(toMap(item));
    }
  }
  ...
  Future<dynamic> create(Map<String, dynamic> data, {String? id}) {
    if (id != null) {
      return _db.collection(basePath).doc(id).set(data);
    } else {
      return _db.collection(basePath).add(data);
    }
  }
  ...
  Future<void> updateData(String id, Map<String, dynamic> data) {
    return _db.collection(basePath).doc(id).update(data);
  }

  /// Removes item with [id] from [collection]
  Future<void> removeItem(String id) {
    return _db.collection(basePath).doc(id).delete();
  }
}
